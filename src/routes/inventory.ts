...
app.get('/api/inventory/variants', async (c) => {
  ...
  const { data, error } = await auth.client
    ...
    .select('id, name, sku, barcode, unit_price, sort_order, inventory_item_id, inventory_items!inner(name, unit, item_kind, image_urls, category_id), inventory_stock(branch_id, quantity_on_hand), inventory_batches(id, branch_id, purchase_price, selling_price, quantity_remaining, received_at)')
    .eq('status', 'active')
