...
export default async function employeesRoutes(app) {
    ...
    server.get('/api/employees', async (request, reply) => {
        ...
        if (profiles.error)
            return sendPgError(reply, profiles.error);
        if (invites.error)
            return sendPgError(reply, invites.error);
    ...
    server.patch('/api/employees/:id', { schema: { params: uuidParam, body: updateBody } }, async (request, reply) => {
        ...
        if (b.department !== undefined)
            patch.department = b.department || null;
        if (b.roleId !== undefined)
            patch.role_id = b.roleId;
